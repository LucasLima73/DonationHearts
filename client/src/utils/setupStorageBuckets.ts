 ...
 * - bucket 'campaign_images': para armazenar imagens de campanhas
 */
export async function setupStorageBuckets() {
  try {
    console.log('Verificando buckets de armazenamento...');
    
    // Verificar se o bucket campaign_images existe
    ...
    // Verificar se o bucket campaign_images já existe
    const campaignImagesBucket = buckets?.find(bucket => bucket.name === 'campaign_images');
    
    if (!campaignImagesBucket) {
      console.log('Criando bucket para imagens de campanhas...');
      
      // Criar o bucket com configurações adequadas
      const { error: createError } = await supabase.storage.createBucket('campaign_images', {
      ...
      if (createError) {
        console.error('Erro ao criar bucket campaign_images:', createError);
        return false;
      }
      
      console.log('✅ Bucket campaign_images criado com sucesso');
      
      // Configurar políticas de acesso público para os arquivos
      try {
        // Política para permitir leitura pública de todos os arquivos
        const { error: policyError } = await supabase.storage.from('campaign_images')
